...
obs_data_path <- "/Users/jagon/Documents/Projects/Collabs/Jessica Badgeley/Jess ITS_LIVE v2/Inputs/Observations/v2/Gl12"
setwd(obs_data_path)
...
##### EGU GRAPH - EXAMPLES DATA #####

# Define folder, year, and elevation
year <- 2020
elevation <- 2
subset_name <- paste0("year", year, "_", elevation)

# Define output folder
output_dir <- "/Users/jagon/Documents/Projects/Collabs/Jessica Badgeley/Jess ITS_LIVE v2/Outputs/Methods/Examples"

# Extract objects
subset_data_obs <- data_with_anomalies_obs[[subset_name]]
subset_data_model <- data_with_anomalies_model[[subset_name]]
subset_residual_data <- residuals_data[[subset_name]]
spline_fit_obs <- spline_fits_obs[[subset_name]]
spline_fit_model <- spline_fits_model[[subset_name]]
spline_fit_residuals <- spline_fits_residuals[[subset_name]]

# Helper function to save each object to CSV
save_csv <- function(data, name) {
  file_path <- file.path(output_dir, paste0(folder_name, "_", year, "_", elevation, "_", name, ".csv"))
  write.csv(data, file = file_path, row.names = FALSE)
}

# Save each dataset
save_csv(subset_data_obs, "subset_data_obs")
save_csv(subset_data_model, "subset_data_model")
save_csv(subset_residual_data, "subset_data_residual")
save_csv(spline_fit_obs, "spline_fit_obs")
save_csv(spline_fit_model, "spline_fit_model")
save_csv(spline_fit_residuals, "spline_fit_residuals")


##### EGU GRAPH - DRIVERS #####

# Define the path to save the PDF
pdf_file_path <- file.path("/Users/jagon/Documents/Projects/Collabs/Jessica Badgeley/Jess ITS_LIVE v2/Outputs/Methods",
                           paste0(folder_name, "_egu_drivers.pdf"))

# Open a PDF device to save the plot (16x5 inches, landscape orientation)
pdf(pdf_file_path, width = 8, height = 2.5)


# Set year and elevation band
year <- 2018
elevation <- 1  # 100 m
subset_name <- paste0("year", year, "_", elevation)
...
all_anomalies <- c(
  subset_data_obs$anomaly,
  subset_data_model$anomaly,
  subset_residual_data$residual_anomaly
)
all_anomalies <- all_anomalies[!is.na(all_anomalies)]
y_max <- max(abs(all_anomalies), na.rm = TRUE)
y_min <- -y_max

# Custom x-axis positions (approximate)
x_ticks <- c(0.1667, 0.5, 0.836)
x_labels <- c("Mar", "Jul", "Nov")

# Plotting layout with bigger spacing and margins
...
if (!is.null(spline_fit_obs)) {
  lines(spline_fit_obs$date, spline_fit_obs$fitted_anomaly, col = "black", lwd = 2)
}
abline(h = 0, col = "gray40", lty = 3)
axis(1,
     at = as.Date(c(paste0(year, "-03-01"),
                    paste0(year, "-07-01"),
                    paste0(year, "-11-01"))),
     labels = c("Mar", "Jul", "Nov"),
     las = 1)
axis(2, at = c(y_min, 0, y_max), labels = c("-1.0", "0.0", "1.0"), las = 1)
mtext("scaled anomaly", side = 2, line = 2.25, cex = 0.7)
mtext("full seasonality", side = 3, line = 1.75, cex = 0.7)
mtext("(observed velocity)", side = 3, line = 0.75, cex = 0.7)
mtext("=", side = 4, line = 0.8, cex = 2, las = 1)
...
plot(interpolated_points$date, interpolated_points$anomaly, xlab = "", ylab = "", main = "",
     ylim = c(y_min, y_max),  # Adjusted ylim
     xlim = as.Date(c(paste0(year, "-01-01"), paste0(year, "-12-31"))),
     yaxt = "n", xaxt = "n", pch = 1, col = adjustcolor("#ffb09c", alpha.f = 0.5), cex = 0.5)
     
if (nrow(original_points) > 0) {
  points(original_points$date, original_points$anomaly,
         col = adjustcolor("red", alpha.f = 1), pch = 1, cex = 0.6)
}
if (!is.null(spline_fit_model)) {
  lines(spline_fit_model$date, spline_fit_model$fitted_anomaly,
        col = adjustcolor("red", alpha.f = 0), lwd = 1, lty = 1)
  polygon(c(spline_fit_model$date, rev(spline_fit_model$date)),
          c(spline_fit_model$upper, rev(spline_fit_model$lower)),
          col = adjustcolor("red", alpha.f = 0), border = NA)
}
...
axis(2, at = c(y_min, 0, y_max), labels = c("-1.0", "0.0", "1.0"), las = 1)
mtext("scaled anomaly", side = 2, line = 2.25, cex = 0.7)
mtext("ice fronts", side = 3, line = 1.75, cex = 0.7)
mtext("(modelled velocity)", side = 3, line = 0.75, cex = 0.7)
mtext("+", side = 4, line = 0.8, cex = 2, las = 1)

# === Plot 3: RESIDUAL ===
plot(subset_residual_data$date, subset_residual_data$residual_anomaly, xlab = "", ylab = "", main = "",
     ylim = c(y_min, y_max),  # Adjusted ylim
     xlim = as.Date(c(paste0(year, "-01-01"), paste0(year, "-12-31"))),
     yaxt = "n", xaxt = "n", pch = 1, col = adjustcolor("blue", alpha.f = 0.5), cex = 0.5)

if (!is.null(spline_fit_residuals)) {
  lines(spline_fit_residuals$date, spline_fit_residuals$fitted_residual, col = "blue", lwd = 2)
}
abline(h = 0, col = "gray40", lty = 3)
axis(1,
     at = as.Date(c(paste0(year, "-03-01"),
...
axis(2, at = c(y_min, 0, y_max), labels = c("-1.0", "0.0", "1.0"), las = 1)
mtext("scaled anomaly", side = 2, line = 2.25, cex = 0.7)
mtext("friction at base", side = 3, line = 1.75, cex = 0.7)
mtext("(residual velocity)", side = 3, line = 0.75, cex = 0.7)
...
# Define the path to save the PDF
pdf_file_path <- file.path("/Users/jagon/Documents/Projects/Collabs/Jessica Badgeley/Jess ITS_LIVE v2/Outputs/Methods",
                           paste0(folder_name, "_egu_areas.pdf"))
...
# Custom x-axis positions (approximate)
x_ticks <- as.Date(c(paste0(year, "-03-01"), paste0(year, "-07-01"), paste0(year, "-11-01")))
x_labels <- c("Mar", "Jul", "Nov")
...
if (!is.null(subset_data_model)) {
  ...
  if (nrow(original_points) > 0) {
    points(original_points$date, original_points$anomaly,
           col = adjustcolor("red", alpha.f = 1), pch = 0.5, cex = 0.5)
  }
}

if (!is.null(spline_fit_model)) {
  lines(spline_fit_model$date, spline_fit_model$fitted_anomaly, col = "red", lwd = 2)
}

# Draw red polygon between obs and model spline (align on common date grid)
if (!is.null(spline_fit_obs) && !is.null(spline_fit_model)) {
  # Define a common sequence of dates (daily resolution)
  date_seq <- seq(from = as.Date(paste0(year, "-01-01")),
                  to = as.Date(paste0(year, "-12-31")), by = "day")
  
  # Interpolate both splines onto the same date grid
  obs_interp <- approx(spline_fit_obs$date, spline_fit_obs$fitted_anomaly, xout = date_seq)$y
  model_interp <- approx(spline_fit_model$date, spline_fit_model$fitted_anomaly, xout = date_seq)$y
  
  # Plot red polygon between observed and modelled splines
  polygon(x = c(date_seq, rev(date_seq)),
          y = c(obs_interp, rev(model_interp)),
...
abline(h = 0, col = "gray40", lty = 3)
axis(1, at = x_ticks, labels = x_labels, las = 1)
axis(2, at = c(y_min, 0, y_max), labels = c("-1.0", "0.0", "1.0"), las = 1)
mtext("scaled anomaly", side = 2, line = 2.25, cex = 0.7)
mtext("frontal control", side = 3, line = 1.75, cex = 0.7)
mtext(expression(italic(A[front])), side = 3, line = 0.75, cex = 0.7)

# === Plot 2: BLANK PANEL ===
plot(NULL, xlim = c(0, 1), ylim = c(y_min, y_max), xaxt = "n", yaxt = "n", xlab = "", ylab = "", bty = "n")

# === Plot 3: OBSERVED + RESIDUAL ===
plot(subset_data_obs$date, subset_data_obs$anomaly, xlab = "", ylab = "", main = "",
     ylim = c(y_min, y_max),
     xlim = as.Date(c(paste0(year, "-01-01"), paste0(year, "-12-31"))),
     yaxt = "n", xaxt = "n", pch = 1, col = adjustcolor("black", alpha.f = 0.5), cex = 0.5)
...
if (!is.null(subset_residual_data)) {
  points(subset_residual_data$date, subset_residual_data$residual_anomaly,
         col = adjustcolor("blue", alpha.f = 0.5), pch = 1, cex = 0.5)
}
...
# Draw blue polygon between obs and residual spline (align on common date grid)
if (!is.null(spline_fit_obs) && !is.null(spline_fit_residuals)) {
  # Define a common sequence of dates
  date_seq <- seq(from = as.Date(paste0(year, "-01-01")),
                  to = as.Date(paste0(year, "-12-31")), by = "day")
  
  # Interpolate both splines onto the same date grid
  obs_interp <- approx(spline_fit_obs$date, spline_fit_obs$fitted_anomaly, xout = date_seq)$y
  residual_interp <- approx(spline_fit_residuals$date, spline_fit_residuals$fitted_residual, xout = date_seq)$y
  
  # Plot blue polygon between observed and residual splines
  polygon(x = c(date_seq, rev(date_seq)),
          y = c(obs_interp, rev(residual_interp)),
...
axis(2, at = c(y_min, 0, y_max), labels = c("-1.0", "0.0", "1.0"), las = 1)
mtext("scaled anomaly", side = 2, line = 2.25, cex = 0.7)
mtext("basal-frictional control", side = 3, line = 1.75, cex = 0.7)
mtext(expression(italic(A[base])), side = 3, line = 0.75, cex = 0.7)
